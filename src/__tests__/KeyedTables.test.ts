...
it("picks the provider with available keys", () => {
    ...
    const keyedTables = Array.from(findTablesForPattern(graph, toTuple('a=1 data'))).map(match => match[0]);
    expect(keyedTables.length).toEqual(1)
    expect(keyedTables[0].schema.stringify()).toEqual('a(key) data')

    const aKeyTable = keyedTables[0];

    expect(Array.from(findTablesForPattern(graph, toTuple('a data'))).map(match => match[0])).toEqual([aKeyTable]);
