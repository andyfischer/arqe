...
test('get works when accessing inherited tags', async ({run}) => {
    ...
    await run('set a == 1');
    expect(await run('get a branch/123')).toEqual('1')

    await run('set a branch/123 == 2')
    expect(await run('get a branch/123')).toEqual('2');
    expect(await run('get a branch/456')).toEqual('1')
});
...
test('works with double inherit tags', async ({run}) => {
    ...
    expect(await run('get a branch/1')).toEqual('1');
    expect(await run('get a branch2/1')).toEqual('1');
    expect(await run('get a branch/1 branch2/1')).toEqual('1');

    await run('set a branch/1 == 2');

    expect(await run('get a')).toEqual('1');
    expect(await run('get a branch/1')).toEqual('2');
    expect(await run('get a branch2/1')).toEqual('1');
