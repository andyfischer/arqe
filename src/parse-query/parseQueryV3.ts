...
type QueryHandler = (query: Query) => Promise<any>
...
async function queryExpression(cxt: Context, it: TokenIterator): Promise<QueryExpr> {
    ...
    await cxt.handler(out);

    return out;
}

async function barInfixedExpression(cxt: Context, it: TokenIterator): Expr {
    const firstToken = it.next();
    ...
    while (it.nextIs(t_bar)) {
        it.consume(t_bar);
        exprs.push(await queryExpression(cxt, it));
    }
...
function runRule(cxt: Context, it: TokenIterator, rule: string) {
    if (rule === 'statement') {
        return statement(cxt, it)
    } else if (rule === 'statements-file') {
        return statementsFile(cxt, it);
    } else {
...
export function parseString(input: string, firstRule: string): ParsedQuery {
    const cxt = new Context();
...
export function parseStatement(input: string, handler: QueryHandler) {
    const cxt = new Context();
    cxt.handler = handler;
    const { iterator } = tokenizeString(input);
    statement(cxt, iterator);
}
*/
