...
const _1 = require(".");
_1.test('can get on an object-space column', async ({ run }) => {
...
_1.test('ObjectSpace attributes exclude multiple values', async ({ run }) => {
    await run('set object-type/ot');
    await run('set object-type/ot attribute/attr1');
...
_1.test('can set (unique) with objects', async ({ run, graph }) => {
    await run('set object-type/ot');
    expect(await run('set ot/(unique)')).toEqual(['ot/ot-1']);
});
_1.test('errors with unexpeced expr', async ({ run }) => {
    await run('set object-type/ot');
    expect(await run('set ot/(something-else)')).toEqual("#error unexpected expression: (something-else)");
});
_1.test('errors with unrecognized attribute', async ({ run }) => {
    await run('set object-type/ot');
    expect(await run('set ot/ot3 notattr/1')).toEqual("#error object type 'ot' has no attribute 'notattr'");
});
_1.test('can get multiple attributes at once', async ({ run }) => {
    await run('set object-type/ot');
    await run('set object-type/ot attribute/attr1');
...
_1.test('getting a nonexisting object returns nothing', async ({ run }) => {
    await run('set object-type/ot');
    expect(await run('get ot/ot5')).toEqual('#null');
});
_1.test('getting attributes from a nonexisting object returns nothing', async ({ run }) => {
    await run('set object-type/ot');
    expect(await run('get ot/ot6 attr/a')).toEqual('#null');
});
_1.test('implicitly creates an object during attribute assignment', async ({ run }) => {
    await run('set object-type/ot');
    await run('set ot/ot7 foo/a');
    expect(await run('get ot/ot7')).toEqual('#exists');
});
_1.test('supports object search by value', async ({ run }) => {
    await run('set object-type/ot');
...
//# sourceMappingURL=ObjectSpace.disabled.js.map