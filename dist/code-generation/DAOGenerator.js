...
function javascriptTemplate(vars) {
    return (`import { GraphLike, Relation, receiveToRelationListPromise } from '${vars.fsImportPath}'
...
class DAOGenerator {
    ...
    getTypeForListenerCallback(touchpoint) {
        const tagValueOutput = this.api.touchpointTagValueOutput(touchpoint);
        const tagOutput = this.api.touchpointTagOutput(touchpoint);
        if (tagValueOutput)
    ...
    relationOutputExpression(touchpoint) {
        const tagValueOutput = this.api.touchpointTagValueOutput(touchpoint);
        const tagOutput = this.api.touchpointTagOutput(touchpoint);
        if (tagValueOutput)
...
function runDAOGenerator(graph, target) {
...
exports.runDAOGenerator = runDAOGenerator;
