...
const _1 = require(".");
_1.test('returns correct data when inherit tags are used', async ({ run }) => {
...
_1.test('get works when accessing inherited tags', async ({ run }) => {
    await run('set typeinfo/branch .inherits');
...
_1.test('inherit works with double inherit tags', async ({ run }) => {
    await run('set typeinfo/branch .inherits');
...
_1.test('inherit correctly prioritizes multiple inherits', async ({ run }) => {
...
_1.test('inherit respects tag ordering', async ({ run }) => {
...
_1.test('inherit works with valueless tags', async ({ run }) => {
    await run('set typeinfo/branch .inherits');
    await run('set typeinfo/branch2 .inherits');
    await run('set a == 1');
