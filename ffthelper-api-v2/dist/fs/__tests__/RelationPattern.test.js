...
describe('isSupersetOf', () => {
    function test(superPattern, subPattern) {
        return Pattern_1.parsePattern(superPattern).isSupersetOf(Pattern_1.parsePattern(subPattern));
    }
    it('works', () => {
...
describe("matches", () => {
    function test(pattern, relation) {
        const p = Pattern_1.parsePattern(pattern);
        const rel = Pattern_1.parsePattern(relation);
...
describe('freeze', () => {
    it('blocks setValue', () => {
        const pattern = Pattern_1.parsePattern('1 2 3');
        pattern.setValue(1);
        pattern.freeze();
    ...
    it('freezes the tags list', () => {
        const pattern = Pattern_1.parsePattern('1 2 3');
        expect(() => { pattern.tags.push(PatternTag_1.newTag('x')); }).not.toThrow();
        expect(() => { pattern.tags[0].tagType = 'x'; }).not.toThrow();
        pattern.freeze();
        expect(() => { pattern.tags.push(PatternTag_1.newTag('x')); }).toThrow();
...
describe('copy', () => {
    ...
    it("doesn't share the tags object", () => {
        const original = Pattern_1.parsePattern('1 2 3');
        original.freeze();
